...
def mapping_with_pose(
    ...
    for frame_id in range(1,sub_seq_py.seq_len):
        ...
        depth_pr_1 = fb_out['predict_depth']

        kf_shape = kf_shape = np.shape(sub_seq_py.get_image(frame=0))
        dummy_keyframe = np.full(kf_shape, 0.0)

        feed_dict_2 = {
            fb_depth_net.placeholders['image_key']: dummy_keyframe,
            fb_depth_net.placeholders_state['cv']:cv,
            fb_depth_net.placeholders_state['depth_label_tensor']:depth_label_tensor,
        }
        fb_out_2 = session.run(fb_depth_outputs, feed_dict=feed_dict_2)

        depth_pr_2 = fb_out_2['predict_depth']
        
        diff_pr = np.subtract(depth_pr_1, depth_pr_2)
    ...
    for iteration in range(nb_iterations_num):
        ...
        cv, depth_label_tensor = create_cv_conf_from_sequence_py(depth_pr_1, sub_seq_py, session, cv_nb_generate_net, cv_nb_generate_outputs)
        ...
        nb_out_org = session.run(nb_depth_outputs, feed_dict=feed_dict_org)
        depth_pr_org = nb_out_org['predict_depth']

        feed_dict_refine = {
            nb_refine_depth_net.placeholders['image_key']: sub_seq_py.get_image(frame=0),
            nb_refine_depth_net.placeholders['depth_key']: depth_pr_org,
            nb_refine_depth_net.placeholders_state['cv']:cv,
        }
        nb_refine_out_org = session.run(nb_refine_depth_outputs, feed_dict=feed_dict_refine)
        depth_pr_org = nb_refine_out_org['predict_depth'] 


        kf_shape = np.shape(sub_seq_py.get_image(frame=0))
        dummy_keyframe = np.full(kf_shape, 0.0)

        feed_dict = {
            nb_depth_net.placeholders['image_key']: dummy_keyframe,
            nb_depth_net.placeholders_state['cv']:cv,
            nb_depth_net.placeholders_state['depth_label_tensor']:depth_label_tensor,
        }
        ...
        feed_dict = {
            nb_refine_depth_net.placeholders['image_key']: dummy_keyframe,
        ...
        diff_pr_nb = np.subtract(depth_pr_org, depth_pr)
        # dm = compute_depth_metrics(1/depth_gt, 1/depth_pr)
        update_visualization(axes,
                             sub_seq_py.get_image(0),
                             sub_seq_py.get_image(frame=frame),
